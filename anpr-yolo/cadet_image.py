...
def create_image(size, color):
    return Image.new(mode='L', color=color, size=size)

def generate_text_images(texts, text_width, fill, rotation, fontname='Helvetica.ttc'):
    images = []
    for atext in texts:
        # -- Get text image size.
...
def main_generated_samples():
    for i in range(5):
        image, texts, bounds = generate_image(size=(WIDTH, HEIGHT), texts=TEXTS*3, text_width=(7,9),
                                              color=(32,128), fill=(168,255), rotation=(-3,3))
