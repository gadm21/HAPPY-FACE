...
def generator(batch):
    while True:
        ...
        for i in range(batch): # Batch
            image, texts, bounds = generate_image(size=(WIDTH, HEIGHT), texts=TEXTS*3, text_width=(10,10),
                                                  color=(0,0), fill=(255,255), rotation=(0,0))
            # Image.
...
def main():
    ...
    x_devs, y_devs = next(generator(batch=8))
    # Train.
    ...
    for i, (x, r) in enumerate(zip(x_devs, results)):
        ...
        image.save('outputs/aero_{:%Y%m%d-%H%M}_{:02d}.png'.format(now, i))
