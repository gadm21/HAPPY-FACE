...
def generate_text_images(texts, text_width, color, fill, rotation, fontname='Helvetica.ttc'):
    images = []
    for atext in texts:
        # -- Get text image size.
        ...
        # -- Create text image.
        aimage = Image.new(mode='L', size=(w,h), color=color)
        adraw  = ImageDraw.Draw(aimage)
        adraw.text((-x_offset,-y_offset), atext, font=font, fill=fill)
        # -- Rotation
...
def generate_image(size, texts, text_width, color=(0,0), fill=(255,255), rotation=(0,0)):
    ...
    # Background image.
    background = create_image(size=size)
    # Text images.
    images = generate_text_images(texts, text_width=text_width, color=color, fill=fill, rotation=rotation)
    # Paste text images.
...
def main_text_samples():
    images = generate_text_images(TEXTS, text_width=(100,100), color=128, fill=255, rotation=0)
    for i, aimage in enumerate(images):
        aimage.save('cameras-trains/text_{:02}.png'.format(i))

def main_generated_samples():
    for i in range(5):
        image, texts, bounds = generate_image(size=(WIDTH, HEIGHT), texts=TEXTS, text_width=(14,16),
                                              color=(0,16), fill=(128,255), rotation=(-0,0))
