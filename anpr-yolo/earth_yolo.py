...
from earth_image import *

TEXTS = [i for i in '0123456789']
...
def loss(fact, pred):
    ...
    return K.sum(conf_loss + box_loss, axis=-1)
...
def generator(batch):
    while True:
        ...
        for i in range(batch): # Batch
            image, texts, bounds = generate_image(size=(WIDTH, HEIGHT), texts=TEXTS, text_width=(13,15),
                                                  color=(0,16), fill=(128,255), rotation=(-0,0))
            # Image.
            x = np.array(image) / 255
            # x = util.random_noise(x, var=random.uniform(0.001, 0.01)) # Add noise
...
def main():
    ...
    for i, (x, r) in enumerate(zip(x_devs, results)):
        ...
        image.save('outputs/denim_{:%Y%m%d-%H%M}_{:02d}.png'.format(now, i))
